function x=ensospe_filterkw(xin,dx,dt,krange,wrange)
...
x=xin;
[nx nt]=size(x);
kg=ensospe_fftkspe(nx,dx)/(2*pi); wg=ensospe_fftkspe(nt,dt)/(2*pi);

kmin=krange(1); kmax=krange(2);
wmin=wrange(1); wmax=wrange(2);

% box 0:
iwmin=ensospe_searchclosest(wg,wmin); iwmax=ensospe_searchclosest(wg,wmax); 
ikmin=ensospe_searchclosest(kg,kmin); ikmax=ensospe_searchclosest(kg,kmax); 
...
iwmin1=ensospe_searchclosest(wg,-wmin); iwmax1=ensospe_searchclosest(wg,-wmax); 
ikmin1=ensospe_searchclosest(kg,-kmin); ikmax1=ensospe_searchclosest(kg,-kmax); 
